 ...
 * `systemPrompt`. A trace with NO prompt surface at all fails as a channel
 * absence — "cannot see the prompt" is refused to be read as "the prompt
 * lacks the substring".
 */
export function systemPromptIncludes(substring) {
  return {
    ...
    check(trace) {
      ...
      return {
        ok: false,
        message: 'no system prompt surface in the trace: requests happened but no system/message events did'
          + ' — the prompt-surface channel is absent (the seam moved);'
